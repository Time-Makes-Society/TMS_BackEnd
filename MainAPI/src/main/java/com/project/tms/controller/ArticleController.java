...
public class ArticleController {
    ...
    /*@GetMapping("/uuid")
    public ResponseEntity<List<UUIDArticleDTO>> getUUIDArticlesByCategories(@RequestParam(value = "category", required = false) String[] categories,
                                                                            @RequestParam(value = "page", defaultValue = "0") int page,
                                                                            @RequestParam(value = "target", required = false) String targetTime,
                                                                            Pageable pageable) {
        // 카테고리 매개변수가 null이거나 비어 있는지 확인하고 그렇다면 빈 배열로 설정합니다.
        if (categories == null || categories.length == 0) {
            categories = new String[]{""}; // 기본적으로 빈 카테고리를 설정합니다.
        }

        // 최대 페이지 크기를 설정합니다.
        int maxPageSize = 5;
        int adjustedPageSize = Math.min(pageable.getPageSize(), maxPageSize);

        // 지정된 카테고리 및 대상 시간을 기반으로 가장 가까운 기사를 찾는 메서드를 호출합니다.
        List<UUIDArticleDTO> closestArticles = articleService.findClosestToTargetTimeByCategories(categories, targetTime, adjustedPageSize, page);
