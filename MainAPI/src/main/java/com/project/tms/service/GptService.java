...
public class GptService {
    ...
    // 프롬프트 엔지니어링 메서드
    public ChatRequest restartEngineerPrompt(String articleContent) {
        // ChatRequest 객체 생성
        ChatRequest request = new ChatRequest();
        request.setModel("gpt-3.5-turbo");
        ...
        // 사용자 쿼리 시작 지점 정의
        messages.add(new Message("user", startUserQuery(articleContent)));
        messages.add(new Message("user", "현재 위의 원문 기사의 요약 내용이 좋지 않으므로 다시 요약해줘"));
...
//        String url = "http://localhost:8082/article/similarity/" + uuid;
    ...
    public void reCalculateAndSaveGptDiffContent(UUIDArticle uuidArticle) {
        // 요약된 내용 저장
        uuidArticleRepository.save(uuidArticle);

        // 새로운 요약 내용 임베딩 값 저장
        calculateAndSaveGptContentEmbedding(uuidArticle);

        // 유사도 계산 후 저장
        Long similarity = calculateEmbeddingDiffContent(uuidArticle.getId());
        uuidArticle.setDiffContentEmbedding(similarity);
        uuidArticleRepository.save(uuidArticle);
    }
}
